...
def is_bot_running(script_owner_id, file_name):
    """Return True only when the registered child process is genuinely alive.
    A dead/zombie process is removed from the registry, but its log file is kept.
    """
    ...
    try:
        ...
        # Process really ended. Do not delete its log file; it is needed by Logs/Fix.
        logger.warning(f"Process {process.pid} for {script_key} exited (returncode={rc}). Cleaning registry entry only.")
        with PROCESS_LOCK:
            bot_scripts.pop(script_key, None)
        return False
    except Exception as e:
...
def get_script_env(user_folder):
    """Build an isolated environment for hosted scripts.

    User-installed Python packages are placed in a per-user .packages folder.
    This avoids relying on the hosting bot's virtualenv/site-packages and makes
    the exact same packages visible to both the pre-check and the long-running
    child process.
    """
    env = os.environ.copy()
...
def attempt_install_pip(module_name, message, user_folder=None):
    package_name = TELEGRAM_MODULES.get(module_name.lower(), module_name)
    if package_name is None:
        logger.info(f"Module '{module_name}' is core. Skipping pip install.")
        return False
    try:
        package_dir = os.path.join(user_folder, '.packages') if user_folder else None
        if package_dir:
            os.makedirs(package_dir, exist_ok=True)
        bot.reply_to(message, f"🐍 Module `{module_name}` not found. Installing `{package_name}`...", parse_mode='Markdown')

        # Install into the user's own package directory, then verify the import
        # using the same interpreter and PYTHONPATH that the hosted script uses.
        command = [sys.executable, '-m', 'pip', 'install', '--disable-pip-version-check', '--no-input']
        if package_dir:
            command += ['--target', package_dir]
        command += [package_name]
        install_env = get_script_env(user_folder) if user_folder else os.environ.copy()
        logger.info(f"Running install: {' '.join(command)}")
        result = subprocess.run(command, capture_output=True, text=True, check=False,
                                encoding='utf-8', errors='ignore', env=install_env)
        ...
        verify_code = (
            ...
            "name=sys.argv[1]; "
            "sys.exit(0 if importlib.util.find_spec(name.split('.')[0]) else 1)"
        )
        verify = subprocess.run([sys.executable, '-c', verify_code, module_name],
                                capture_output=True, text=True, check=False,
                                encoding='utf-8', errors='ignore', env=install_env)
        if verify.returncode != 0:
            logger.error(f"Installed {package_name}, but import '{module_name}' is still unavailable. "
                         f"verify stderr: {verify.stderr.strip()}")
            bot.reply_to(message,
                         f"❌ `{package_name}` installed, but `{module_name}` is still not importable in the hosted environment.")
            return False

        logger.info(f"Installed and verified {package_name} for module {module_name}. Output:\n{result.stdout}")
        bot.reply_to(message, f"✅ Package `{package_name}` (for `{module_name}`) installed and verified.", parse_mode='Markdown')
        return True
    except Exception as e:
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        if attempt == 1:
            ...
            check_proc = None
            try:
                check_proc = subprocess.Popen(check_command, cwd=user_folder, env=get_script_env(user_folder), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='ignore')
                ...
                if return_code != 0 and stderr:
                    ...
                    if match_py:
                        ...
                        if attempt_install_pip(module_name, message_obj_for_reply, user_folder):
        ...
        try:
            ...
            process = subprocess.Popen(
                [sys.executable, script_path], cwd=user_folder, env=get_script_env(user_folder), stdout=log_file, stderr=log_file,
            ...
            threading.Thread(target=monitor_process, args=(script_key, process), daemon=True).start()
        except FileNotFoundError:
...
def monitor_process(script_key, process):
    """Monitor a hosted child process.

    The hosting bot is never stopped when a child script exits.  We keep the
    child's log file intact, record the real return code, and send a short
    diagnostic to the chat that started the script.
    """
    try:
        return_code = process.wait()
        logger.warning(f"Hosted process {process.pid} for {script_key} exited with code {return_code}.")

        with PROCESS_LOCK:
            info = bot_scripts.get(script_key)
            if not info or info.get('process') is not process:
                return
            chat_id = info.get('chat_id')
            file_name = info.get('file_name', script_key)
            log_file = info.get('log_file')

        # Flush the inherited log stream before reading it.
        try:
        ...
        # Read only the tail so a large log can never produce a huge Telegram message.
        tail = ""
        try:
            ...
            if os.path.isfile(log_path):
                with open(log_path, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    size = f.tell()
                    f.seek(max(0, size - 3500), os.SEEK_SET)
                    tail = f.read()
        except Exception as e:
            logger.error(f"Could not read exit log for {script_key}: {e}")

        # Close the parent's handle after the child has exited; the log remains on disk.
        try:
            if log_file and not log_file.closed:
                log_file.close()
        except Exception:
        ...
        # Remove only the running-process registry entry. The uploaded file and log stay.
        with PROCESS_LOCK:
        ...
        if chat_id:
            if return_code == 0:
                message = f"ℹ️ Script `{file_name}` stopped normally (RC: 0)."
            else:
                # Do not guess the cause. Show the actual last log lines.
                clean_tail = tail.strip() or "(No output was written to the log.)"
                if len(clean_tail) > 3000:
                    clean_tail = clean_tail[-3000:]
                message = (
                    f"❌ Script `{file_name}` exited (RC: {return_code}).\n\n"
                    f"📜 Last log output:\n```\n{clean_tail}\n```\n\n"
                    f"Use View Logs for the complete log."
                )
            try:
                bot.send_message(chat_id, message, parse_mode='Markdown')
            except Exception as e:
                logger.error(f"Could not send process-exit message for {script_key}: {e}")

    except Exception as e:
        logger.error(f"Process monitor failed for {script_key}: {e}", exc_info=True)
