        ...
        try:
            ...
            if process.poll() is not None:
                ...
                bot.reply_to(message_obj_for_reply,
                    f"❌ Script '{file_name}' exited immediately (RC: {rc}). Check logs — likely a bot token conflict or crash in user script.")
                return
        ...
        try:
            ...
            if process.poll() is not None:
                ...
                if script_key in bot_scripts:
                    del bot_scripts[script_key]
                bot.reply_to(message_obj_for_reply,
                    f"❌ JS script '{file_name}' exited immediately (RC: {rc}). Check logs — likely a token conflict or crash in user script.")
                return
    ...
    while True:
        try:
            ...
            if "409" in str(e) or "conflict" in str(e).lower():
                _consecutive_409 += 1
                logger.error(f"409 Conflict #{_consecutive_409}. Another instance running.")
                if _consecutive_409 >= _MAX_409:
                    logger.critical("Too many 409 errors. Hard exit to prevent conflict loop.")
                    cleanup()
                    sys.exit(1)
                time.sleep(15 * _consecutive_409)  # backoff: 15s, 30s, 45s
            else:
                logger.error(f"Telegram API error: {e}. Retrying in 10s...")
                time.sleep(10)
